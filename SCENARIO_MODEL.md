...
- The `near` (근접 메모리 상주) scenario runs two phases when initial loading is included, which is what the lab shows by default — the `journey-near-fill` checkbox and the overview's 단계 0 toggle both ship on, while `simulate()`'s own `nearPrefill` option defaults to off so that callers opt in: phase 0 reads the tile once from HBM to populate the near tier, charged as HBM fixed delay + HBM transfer with no separate near-side write time; phase 1 then serves the same tile from the near tier. Turning the toggle off returns to the already-populated assumption above. The `compare` scenario keeps the already-populated assumption in both settings. Normal L2 stays at 0 B in this scenario because the near path bypasses it and this small example issues no normal-L2 request; that zero is a property of the example, not a measurement. The HBM backing copy remains in both added and partition modes.
...
`node tests/cache-journey.test.cjs` checks repeated-read conservation, hits/misses, LRU eviction, dirty write-back ordering, capacity bounds, zero capacity, bandwidth sensitivity, and the near-tier initial fill (phase ordering, fill bytes, the HBM read it charges, its duration, that `simulate()`'s `nearPrefill` option is off unless a caller passes it, that no fill happens when near capacity is unavailable, and that the fill's bytes are the same bytes it charges to `hbmRead` rather than additional traffic). Browser checks cover playback completion, dirty eviction (16 MiB written back), live storage colors, timing changes, preset changes and console errors.
...
- Recorded cases have two layers. **실측만** (the default) draws static routes, hatched unknown occupancy and the recorded timing only; it animates nothing and locks the playback-speed control, because no per-address movement was recorded. **Ledger 재구성** animates an estimated path built from object sizes, the reservation and the logical access counts, with dashed lines, hollow dashed dots and a `재구성(실측 아님)` badge. The reconstruction never reads the recorded step time, and its bytes are labeled `재구성` wherever they are shown.
- Proxy reconstruction applies the placement model to the experiment's own aligned `object_bytes` and `reserve_bytes`: state placement fraction `min(1, reserve/object)`, persisting service `2 × object × fraction`, reconstructed HBM read `weight + KV + state × (1 − fraction)` and write `state × (1 − fraction)`. It assumes no reuse between steps and uniform access inside a placed object, so it is an upper bound on HBM traffic, not a counter reading.
- Decode reconstruction shows the same structural per-step traffic the whole-model calculator already computes for that model at B=8, context 2048, with no near memory. The recorded `step_ms` is not decomposed into these link times.
- Aggregate measured flows: if a decode record carried an aggregate DCGM HBM-byte field, it would be drawn as a single gray flow, never split into weight/KV/state colors. The current `assets/model_evidence.json` decode records carry only batch, context, `step_ms`, power, throughput, energy, `decode_steps` and `timing_intervals_used`, so that field is absent and the page leaves the quantity explicitly 미계측. The Granite `state_traffic_estimation` is a separate regression with `direct_read_write_counter: false` in units of bytes per resident state byte; it is not a per-step HBM byte count.

## Persisting reservation wording

The protected region is labeled `L2 persisting 예약 영역`, and the policies are `state / KV / weight persisting 힌트`. The page states inline that this is the `cudaLimitPersistingL2CacheSize` + `cudaAccessPolicyWindow` hint, that it does not pin a line or guarantee residency, and that unused set-aside capacity remains usable by normal accesses. Raw-data policy ids (`persist_state`, `persist_kv`, `persist_weight`, `reserved_normal`, `default`) are unchanged in every JSONL, summary and analysis script.
