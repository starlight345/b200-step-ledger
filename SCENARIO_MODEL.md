...
`node tests/cache-journey.test.cjs` checks repeated-read conservation, hits/misses, LRU eviction, dirty write-back ordering, capacity bounds, zero capacity, bandwidth sensitivity, and the near-tier initial fill (phase ordering, fill bytes, the HBM read it charges, its duration, and that it stays off by default and when near capacity is unavailable). Browser checks cover playback completion, dirty eviction (16 MiB written back), live storage colors, timing changes, preset changes and console errors.
