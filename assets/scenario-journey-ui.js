(function () {
  ...
  function config() {
    ...
    return {capacityBytes:J.L2_BYTES-reserve,tileBytes:tile,hbmTBps:number('hbm'),l2TBps:number('l2-bandwidth'),
      ...
      nearLabel:partition?'보호 L2':'3D SRAM',primaryKind:$('journey-pattern').value==='dirty'?'state':whole.order[0]||'kv',otherKinds:Object.keys(whole.obj).filter(k=>whole.obj[k].alloc>0),pattern:$('journey-pattern').value};
  }
  ...
  function renderStorage(){
    ...
    $('near-storage-title').textContent=partition?'보호 L2 · 배치한 사본':'3D SRAM · 배치한 사본';
  ...
  function renderTiming(){
    const t=result.timings,max=Math.max(t.hit,t.miss,t.near),c=result.config;
    ...
    $('near-title').textContent=c.nearLabel==='보호 L2'?'보호 L2 · 같은 칩':'가상 3D SRAM';
    $('near-speed').textContent=c.nearTBps.toFixed(1)+' TB/s · '+c.nearLatencyNs+' ns';
    $('near-available').textContent=c.nearAvailable?'A 사전 상주 가정':'용량 부족 · 경로 제외';
    $('near-path-label').textContent=c.nearLabel==='보호 L2'?'일반·보호 영역은 같은 물리 L2':'추가 SRAM 직접 연결 가정';
  ...
  function paint(){
    ...
    if(lastShown!==index){
      $('journey-kind').textContent='요청 '+(e.request+1)+'/'+result.requests.length+' · '+names[req.kind]+' '+req.id+' · '+req.source;
    ...
    const node=e.type==='fetch'||e.type==='writeback'?'hbm':e.type==='near'?'near':e.type==='ready'?'compute':'l2';$('node-'+node).classList.add('active');
