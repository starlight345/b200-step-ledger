...
function modelData(x=calc(),recorded=false){
 ...
 return{type:'model',title:x.m.proxy?'작은 대리실험의 저장량과 이동량':cases.model.title,tag:'가정 + 구조 계산',desc:description,unit:x.m.proxy?'대리실험 한 스텝':'한 스텝',stores:[store('HBM · 지속 데이터',payload,HBM,'구조·가정값 · 실제 할당 덤프 아님'),normalStore,store(partition?'보호 L2 · 같은 물리 캐시':'가상 3D SRAM',placed,nearCap,'사전 배치 가정 · 초기 적재 비용 제외')],flows,
