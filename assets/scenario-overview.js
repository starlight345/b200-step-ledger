...
function draw(d){
 ...
 $('near-fill')?.addEventListener('click',()=>{nearFill=!nearFill;running=false;refresh();});
 document.querySelectorAll('[data-layer]').forEach(el=>el.addEventListener('click',()=>{if(measuredLayer===el.dataset.layer)return;measuredLayer=el.dataset.layer;running=false;refresh();}));
...
function select(key){
 selected=key;cachePhase='final';running=false;conditionUpdated=true;measuredLayer='measured';const c=cases[key];
