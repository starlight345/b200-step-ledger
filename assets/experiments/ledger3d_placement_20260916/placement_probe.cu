...
int main(){
  ...
  const size_t MiB=1024ull*1024ull, objectBytes=64*MiB;
  const int layers=36;
  ...
  if(reserveBytes<usedBytes || (size_t)p.accessPolicyMaxWindowSize<usedBytes){fprintf(stderr,"Equal target does not fit the supported persisting window.\n");return 3;}
  ...
  for(int oi=0;oi<6;oi++) for(int pass=0;pass<3;pass++){
    ...
    for(int po=0;po<5;po++){
      ...
      attr.accessPolicyWindow.num_bytes=pol.target>=0?usedBytes:0;
      attr.accessPolicyWindow.hitRatio=pol.target>=0?1.0f:0.0f;
